...
function AgregarForm() {
  ...
  const [voiceTarget, setVoiceTarget] = useState<'snippet' | 'contexto'>('snippet');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!user || !habilidad || !snippet) return;

    setSaving(true);
    try {
      const data = {
        habilidad,
        snippet_conocimiento: snippet,
        contexto_uso: contexto,
        multimedia_ref: multimedia,
        tags: tags
          .split(',')
          .map((t) => t.trim())
          .filter(Boolean),
  ...
  const handleVoice = (target: 'snippet' | 'contexto') => {
